...
async function createOtpByEmail(email, otp, purpose, expiresAt) {
  const userResult = await pool.query(
    `SELECT id
     FROM users
...
async function findLatestOtpByEmailAndPurpose(email, purpose) {
  const result = await pool.query(
    `SELECT o.id, o.user_id, o.otp_code, o.purpose, o.expires_at, o.is_used, o.created_at
     FROM otps o
     INNER JOIN users u ON u.id = o.user_id
     WHERE LOWER(u.email) = LOWER($1) AND o.purpose = $2
     ORDER BY o.created_at DESC
     LIMIT 1`,
    [email, purpose]
  );
...
module.exports = {
  ...
  findLatestOtpByEmailAndPurpose,
  markOtpAsUsed
};
